...
```{r costum functions}
tidy_world_bank_data <- function(.dataframe){
...
filter_arab_countries <- function(.dataframe){
  
  arab_countries <- .dataframe %>% 
...
p_events <- function(.dataframe){
   
  plot_coordinates <- data_frame(
    xmin = min(timeline$Date),
    xmax = max(timeline$Date),
    ymin = min(.dataframe[3], na.rm = T),
    ymax = max(.dataframe[3], na.rm = T)
  )
...
```{r costum plot aesthetics}


# adjust styling of a plot
p_x_scale <- list(
  labs(color = "Country"),
  xlab("Year"),
  theme_minimal(),
...
gini_plus1 <- gini %>% 
  filter_arab_countries() %>% 
  filter(!is.na(gini))

ggplot(gini_plus1,
       aes(x = Year, y = gini, color = Country_Name)) +
  geom_point() + 
  geom_line() +
  p_x_scale + 
  p_events(gini_plus1) +
...
```{r plot other taxes}
# remove NAs for correct adjustment of x and y axis
other_taxes_plus1 <- other_taxes %>% 
  filter_arab_countries() %>% 
  filter(!is.na(other_taxes))

ggplot(other_taxes_plus1, 
       aes(x = Year, other_taxes, color = Country_Name)) + 
  geom_point() + 
  geom_line() +
  p_events(other_taxes_plus1) +
  p_x_scale
...
```{r}
# remove NAs for correct adjustment of x and y axis
taxes_on_income_profits_capital_plus1 <- taxes_on_income_profits_capital %>% 
  filter_arab_countries() %>% 
  filter(!is.na(taxes_on_income_profits_capital))

ggplot(taxes_on_income_profits_capital_plus1, 
       aes(x = Year, taxes_on_income_profits_capital, color = Country_Name)) + 
  geom_point() + 
  geom_line() +
  p_events(taxes_on_income_profits_capital_plus1) +
  p_x_scale
```
...
```{r}
# remove NAs for correct adjustment of x and y axis
taxes_on_international_trade_plus1 <- taxes_on_international_trade %>% 
  filter_arab_countries() %>% 
  filter(!is.na(taxes_on_international_trade))

ggplot(taxes_on_international_trade_plus1, 
       aes(x = Year, taxes_on_international_trade, color = Country_Name)) + 
  geom_point() + 
  geom_line() +
  p_events(taxes_on_international_trade_plus1) +
  p_x_scale
```
