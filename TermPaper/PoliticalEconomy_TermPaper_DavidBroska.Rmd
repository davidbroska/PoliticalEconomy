...
However, the extent to which the elites are able to profit from their political power is restricted by the danger that other groups might attempt to overthrow the incumbent regime. It follows that the danger of a revolution puts a constraint on the policies of the elites in nondemocracy. In the first place, Acemoglu and Robinson argue that the likelyhood of a revolution depends on the citizens income $y^p$. In a nondemocracy $N$ the tax rate $\tau^r$ is set by the elites so that the utility of the poor is given by
$$V^p=y^p=(y^p|\tau^N = \tau^r).$$
However, revolutions are costly not only for the elite that looses their privileges in the existing regime. It is reasonable that revolutionary events imply disorder and distruction, so that the productivity of the economy shrinks. Stated differently, there is a fraction $\mu$ that is destroyed during the revolution so that the rest $1-\mu$ can be distributed among the poor. Thus, the utility $V^P(R, \mu)$ of a poor agent in the postrevolutionary society, condition on that fraction $\mu$, is given by
$$ V^P(R, \mu)=\frac{(1-\mu)\bar{y}}{1-\delta}.$$
This inidicates, that total income available denoted by $(1-\mu)\bar{y}$ will be devided among the poor population $1-\delta$. From these equation Acemoglu and Robinson derive the first result for nondemocracies: The extent to which the elites can define policies to their benefit is restricted by the utility of the poor. In other words, the revolution constraint binds if the potential income of a poor in a postrevolutionary society is greater than his current income: 
$$ \frac{(1-\mu)\bar{y}}{1-\delta} > y^p$$
This implies that individuals behave strategically under conditions of complete information. On the one hand, this assumption allows the authors to create a model that allows to make prediction about the likelyhood of a revolution based on economic indicators. On the other hand, the flexibility of the model achieved by this reduction of complexity requires the model to be tested empirically. Would the model have predicted the revolutionary movements of the Arab Spring?
...
wb_data %>% 
  ...
  scale_x_date(limits = c(as.Date("1990-01-01"), as.Date("2017-01-01")),
               labels = scales::date_format("%Y"),
               breaks = '2 years')
```
