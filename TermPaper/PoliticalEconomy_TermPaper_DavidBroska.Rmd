...
```{r aggregated-income-share, out.width="90%", fig.cap="Aggregated income share of lower 80%"}
...
```{r function to extract income share}
# returns a single numeric value
get_income <- function(.df, .Year, .Country_Name, .Series_Name){
...
Although the relevant indicators are only available for Egypt and Djibouti, the World Bank database provides evidence for the hypothesized relation between the distribution of incomes and a countries vulnarability for revolutions. Indeed, figure \@ref(fig:aggregated-income-share) shows that the income share accruing to the lower 80% of Egypt's population increased from `r get_income(income_lower_80, 2010, "Egypt", "lower 80%")`% in 2010 to `r get_income(income_lower_80, 2012, "Egypt", "lower")`% in 2012, one after the major events of the Arab Spring. This development generally supports Acemoglu and Robinson's idea that revolutions occur when citizens can reasonably assume to gain more than they loose. Notably, the incomes of the top 20% deminished in this period from `r get_income(wb_data, 2010, "Egypt", "highest 20%")`% to `r get_income(wb_data, 2012, "Egypt", "highest 20%")`%. Therefore, it seems reasonable that the improvement of the poor was at the expense of the elite. Undoubtedly, this variation might be due to other factors than to the consequences of the revolutionary movements. However, the literature generally supports this interpretation. 
...
```{r plot GDP per capita, fig.cap=details_gdp_capita %>% pull(2)}
